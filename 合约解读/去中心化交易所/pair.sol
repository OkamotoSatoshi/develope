...
contract UniswapV2Pair is UniswapV2ERC20 {
    ...
    event Mint(address indexed sender, uint amount0, uint amount1);  //通知前端
    ...
    function mint(address to) external lock returns (uint liquidity) {
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // 两个token的的数量
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); //两个token的数量
        ...
        if (data.length > 0) IUniswapV2Callee(to).uniswapV2Call(msg.sender, amount0Out, amount1Out, data);  //整个金融史最大的创新 ----闪电贷
        balance0 = IERC20Uniswap(_token0).balanceOf(address(this));
        balance1 = IERC20Uniswap(_token1).balanceOf(address(this));
        }
